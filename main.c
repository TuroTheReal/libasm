...
void test_ft_strdup(const char *test_name, const char *input)
{
	 printf("Test: %s\n", test_name);
	 printf("Input: \"%s\"\n", input ? input : "(null)");
  	if (!input){
...
int main(void) // MADATORY PART
{
	...
	printf("========= Tests pour ft_strdup =========\n\n");

	test_ft_strdup("Chaîne simple", "Hello");
	test_ft_strdup("Chaîne vide", "");
	test_ft_strdup("Un seul caractère", "A");
	test_ft_strdup("Chaîne avec espaces", "Hello World");
	test_ft_strdup("Chaîne longue", "Cette chaîne est suffisamment longue pour tester les performances");
	test_ft_strdup("Caractères spéciaux", "!@#$%^&*()_+-={}[]|\\:;\"'<>,.?/");
	test_ft_strdup("Chiffres", "1234567890");
